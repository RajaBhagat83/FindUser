  ...
  return (
    <div className="w-screen h-screen flex flex-col bg-slate-50 dark:bg-[#0b1120] text-slate-800 dark:text-slate-200 font-sans selection:bg-violet-500/30 transition-colors duration-300">
      ...
      <div className="flex flex-grow overflow-hidden relative z-10">
        <Connection className="w-[30%] min-w-[300px] z-20" />
        
        <div className="flex-1 h-full flex flex-col relative z-10 bg-slate-50/50 dark:bg-[#0b1120]/50 backdrop-blur-sm transition-colors duration-300">
          {messages.receiver ? (
            <>
              {/* ── Receiver header bar */}
              <div className="flex justify-between items-center w-full px-6 py-4 bg-white/90 dark:bg-[#0f172a]/80 backdrop-blur-xl border-b border-slate-200 dark:border-white/5 flex-shrink-0 shadow-sm dark:shadow-sm dark:shadow-black/20 z-30 transition-colors duration-300">
              ...
              {/* ── Messages area */}
              <div className="flex-1 w-full overflow-y-auto px-8 py-6 flex flex-col gap-2 custom-scrollbar">
                {messages.messages.map((msg, idx) => {
                  const isMe = msg.user._id === user._id;
                  return (
                    <div
                      key={idx}
                      className={`flex flex-col max-w-[65%] w-fit mb-2 ${
              ...
              {/* ── Input row */}
              <div className="w-full px-6 py-5 bg-white/90 dark:bg-[#0f172a]/80 backdrop-blur-xl border-t border-slate-200 dark:border-white/5 flex-shrink-0 z-30 transition-colors duration-300">
